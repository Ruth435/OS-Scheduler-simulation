...
# Load the CSV file
data <- read.csv("simulation_summary.csv",header = TRUE, colClasses = c( rep("character",18), rep("NULL",17)))
...
# Identify the metric columns (all except metadata columns)
# Adjust this list if your metadata columns are different
non_metric_cols <- c("algorithm", "quantum", "context_switch_time", "num_patrons", "seed")
metric_cols <- setdiff(names(data), non_metric_cols)

# Now average across each algorithm/context switch time
summary_data <- data %>%
